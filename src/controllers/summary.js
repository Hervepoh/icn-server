...
const all = (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const from = fromDate ? (0, moment_1.default)(fromDate.toString(), 'YYYY-MM-DD').toDate() : defaultFrom;
    const to = toDate ? (0, moment_1.default)(toDate.toString(), 'YYYY-MM-DD').toDate() : defaultTo;
    ...
    const lastPeriod = yield fetchSummaryData(from, to);
...
function fetchSummaryData(from, to) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const countByStatus = yield prismadb_1.default.transaction.groupBy({
            by: ['statusId'],
            where: {
                paymentDate: {
                    gte: from,
        ...
        const totalRequestCount = yield prismadb_1.default.transaction.count({
            where: {
                paymentDate: {
        ...
        const sumAmountByStatus = yield prismadb_1.default.transaction.groupBy({
            by: ['statusId'],
            where: {
                createdAt: {
                    gte: from, // Date de début
        ...
        const requesters = yield prismadb_1.default.transaction.findMany({
            where: {
                createdAt: {
                    gte: from, // Date de début
                    lte: to // Date de fin
        ...
        // }));
        const activeDays = yield prismadb_1.default.transaction.groupBy({
            by: ['createdAt'],
            where: {
                createdAt: {
                    gte: from, // Date de début
...
function fillMissingDays(activeDays, startDate, endDate) {
    ...
    const transactionsByDay = allDays.map((day) => {
        const found = activeDays.find((d) => (0, date_fns_1.isSameDay)(d._id, day));
        if (found) {
            return {
                date: day,
                number: found.count,
