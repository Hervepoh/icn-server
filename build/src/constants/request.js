...
exports.sqlQuery = {
     ...
     *       CMS UNPAID QUERY
    ------------------------------------------------ */
    unpaid_bills_by_contract_number: `select /*+ parallel(4) */
    ...
    unpaid_bills_by_invoice_number: `select /*+ parallel(6) */
        s.regroup_id code_regroupement,
        decode(r.nis_rad, 0, r.cod_cta_pago, r.nis_rad) numero_contrat,
        r.num_rec numero_facture,
        s.cust_name nom_client,
    ...
    where /*r.num_rec in (424378539) or*/ (r.num_rec = :BillNber
        ...
        and ((r.est_act in (select est_rec from cmsadmin.grupo_est where co_grupo = 'GE114')
            ...
            or (r.est_act = 'ER240' and s.est_serv = 'EC020')
            or r.nis_rad = 0))`,
    unpaid_bills_by_customer_regroup_number: `with regroupCusts as (select /*+ parallel(2) */ doc_id, nis_rad from cmsadmin.xclientes x where x.doc_id = :RegroupCode)
		select /*+ parallel(4) */
			doc_id code_regroupement,
			r.nis_rad numero_contrat,
			r.num_rec numero_facture,
		...
		where imp_tot_rec - imp_cta > 0
			...
			and ((r.est_act in (select est_rec from cmsadmin.grupo_est where co_grupo = 'GE114')
				...
				or (r.est_act = 'ER240' and s.est_serv = 'EC020'))
		order by f_prev_puesta desc`,
    unpaid_bills_by_customer_name: `with tb_sdgc_groupes as (select /*+ parallel(2) */ groupe, service_no from cmsadmin.sdgc_groupes x where groupe = :CustomerName)
		select /*+ parallel(4) */
		...
		where imp_tot_rec - imp_cta > 0
			...
			and ((r.est_act in (select est_rec from cmsadmin.grupo_est where co_grupo = 'GE114')
				...
				or (r.est_act = 'ER240' and s.est_serv = 'EC020'))
			order by f_prev_puesta desc`,
    unpaid_bills_on_list: `select /*+ parallel(8) */ distinct
		...
		where b.nis_rad &lt;&gt; 0
			...
			and r.tip_rec = 'TR060' 
    `,
    unpaid_bills_on_list_with_account: `select /*+ parallel(8) */ distinct
			(select distinct s.groupe from cmsadmin.sdgc_groupes s where s.service_no = b.nis_rad) code_regroupement,
			b.nis_rad numero_contrat,
			b.num_rec numero_facture,
			(select regexp_replace(regexp_replace(regexp_replace(c.ape1_cli || ' ' || c.nom_cli || ' ' || c.ape2_cli,'[\+\*\$!%,?;^¨§]*'),
    ...
    /*------------------------------------------------
      *       ICN QUERY
     ------------------------------------------------ */
    icn_next_code: "select cmsadmin.seq_code_aci.nextval codeaci from dual",
    icn_next_dematerialisation_code: "select cmsadmin.seq_code_dematerialisation.nextval code_dematerialisation from dual",
    select_groupes: "select distinct trim(groupe) groupe from cmsadmin.sdgc_groupes order by 1",
    icn_fulldata: ``,
    icn_lightdata: ``,
    icn_infos: `select /*+ parallel(6) */ distinct
			id_cobtemp,
			session_id,
			id_pago,
			nis_rad,
    ...
    icn_search_bill_status_by_aci_number_offline_collections: `SELECT /*+ parallel(6) */ DISTINCT 
			COD_CAJA,
			ACI_NUMBER,
			SESSION_ID  
